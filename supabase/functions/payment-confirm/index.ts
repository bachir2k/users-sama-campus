...
serve(async (req) => {
  ...
  try {
    ...
    const customData = (confirmData.custom_data ?? {}) as { user_id?: string; amount?: number; type?: string }
    ...
    if (customData.user_id !== studentId) {
      console.error('payment-confirm: user mismatch', { tokenUser: customData.user_id, callerStudent: studentId })
